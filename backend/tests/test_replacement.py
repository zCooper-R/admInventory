...
from apps.inventory.services.replacement import CPUCategory, assess_device_for_replacement, classify_cpu
...
class TestReplacementRules:
    def test_replacement_depends_only_on_hardware(self):
        base = DeviceFactory(ram=16, storage_type="SSD", cpu_model="Intel Core i5-12400")
    ...
    def test_is_certified_does_not_affect_score(self):
        a = DeviceFactory(ram=8, storage_type="HDD", cpu_model="Intel Core i3-7100", is_certified=True)
        b = DeviceFactory(ram=8, storage_type="HDD", cpu_model="Intel Core i3-7100", is_certified=False)
        assert assess_device_for_replacement(a).score == assess_device_for_replacement(b).score

    def test_use_flags_do_not_affect_score(self):
        a = DeviceFactory(ram=8, storage_type="HDD", cpu_model="Intel Core i3-7100", use_for_video=False)
        b = DeviceFactory(ram=8, storage_type="HDD", cpu_model="Intel Core i3-7100", use_for_video=True)
        assert assess_device_for_replacement(a).score == assess_device_for_replacement(b).score
    ...
    def test_unknown_cpu_is_handled(self):
        result = assess_device_for_replacement(DeviceFactory(ram=16, storage_type="SSD", cpu_model="My Custom CPU 123"))
        assert result.status in {ReplacementStatus.OK, ReplacementStatus.ATTENTION, ReplacementStatus.REPLACE}
